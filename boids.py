    ...
    def collision_check(self, new_velocity, tree, self_radius, collision_radius):
        ...
        if number_of_collisions > 0:
            ...
            if (number_of_collisions == 1):
                ...
                collision_acc = -0.1*(relative_unit_vector)*np.exp(overlap)
            else:
                ...
                # Calculate collision acceleration (Basically Pauli exclusion).
                collision_acc = np.sum(-0.1*(relative_unit_vector)*np.exp(overlap[:,np.newaxis]),axis=0)/number_of_collisions
