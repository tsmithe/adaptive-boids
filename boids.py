    ...
    def mutate(self):
        ...
        for i in np.arange(network_size):
            if (np.random.random() < self.mutation_probability):
                mutated_weights[i] -= 2*self.creep_range*(np.random.random()-0.5)
